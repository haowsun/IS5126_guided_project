...
class p36m_spider(scrapy.Spider):
    ...
    def parse_performance(self, response):
        ...
        table = response.xpath('//*[starts-with(@id, "all_per_poss")]').extract()[0] \
            if len(response.xpath('//*[starts-with(@id, "all_per_poss")]').extract()) > 0 else ''
        table = re.sub('<!--', '', table)
        table = re.sub('-->', '', table)
        table = Selector(text=table)

        for p in table.xpath('.//tr[starts-with(@id, "per_poss")]'):
            #print(p.extract())
            performance = {}
            ...
            performance['FG'] = self.fun(p, './/*[@data-stat="fg_per_poss"]//text()')
            performance['FGA'] = self.fun(p, './/*[@data-stat="fga_per_poss"]//text()')
            performance['FG%'] = self.fun(p, './/*[@data-stat="fg_pct"]//text()')
            performance['3P'] = self.fun(p, './/*[@data-stat="fg3_per_poss"]//text()')
            performance['3PA'] = self.fun(p, './/*[@data-stat="fg3a_per_poss"]//text()')
            performance['3P%'] = self.fun(p, './/*[@data-stat="fg3_pct"]//text()')
            performance['2P'] = self.fun(p, './/*[@data-stat="fg2_per_poss"]//text()')
            performance['2PA'] = self.fun(p, './/*[@data-stat="fg2a_per_poss"]//text()')
            performance['2P%'] = self.fun(p, './/*[@data-stat="fg2_pct"]//text()')
            performance['FT'] = self.fun(p, './/*[@data-stat="ft_per_poss"]//text()')
            performance['FTA'] = self.fun(p, './/*[@data-stat="fta_per_poss"]//text()')
            performance['FT%'] = self.fun(p, './/*[@data-stat="ft_pct"]//text()')

            performance['ORB'] = self.fun(p, './/*[@data-stat="orb_per_poss"]//text()')
            performance['DRB'] = self.fun(p, './/*[@data-stat="drb_per_poss"]//text()')
            performance['TRB'] = self.fun(p, './/*[@data-stat="trb_per_poss"]//text()')
            performance['AST'] = self.fun(p, './/*[@data-stat="ast_per_poss"]//text()')
            performance['STL'] = self.fun(p, './/*[@data-stat="stl_per_poss"]//text()')
            performance['BLK'] = self.fun(p, './/*[@data-stat="blk_per_poss"]//text()')
            performance['TOV'] = self.fun(p, './/*[@data-stat="tov_per_poss"]//text()')
            performance['PF'] = self.fun(p, './/*[@data-stat="pf_per_poss"]//text()')
            performance['PTS'] = self.fun(p, './/*[@data-stat="pts_per_poss"]//text()')
            performance['ORtg'] = self.fun(p, './/*[@data-stat="off_rtg"]//text()')
            performance['DRtg'] = self.fun(p, './/*[@data-stat="def_rtg"]//text()')

            #print(performance)
            yield performance

        for p in table.xpath('.//tr[starts-with(@id, "playoffs_per_poss")]'):
            # print(p.extract())
            performance = {}
            performance['Name'] = name
            performance['Born'] = born
